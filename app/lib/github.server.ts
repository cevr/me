  ...
  return Task.from(
    async () => {
      const res = (await request(
        `https://api.github.com/repos/cevr/cms/contents/${path}`,
        {
          headers: {
...
): AsyncTask<GetShaFailedError | PushFailedError, void> {
...
function get<T>(path: string): AsyncTask<GetFileFailedError, T> {
  return Task.from(
    () =>
      fetch(`https://raw.githubusercontent.com/cevr/cms/main/${path}`).then(
        (res) => res.json(),
      ),
...
function getDir<T>(
  path: string,
): AsyncTask<GetDirFailedError | GetFileFailedError, T[]> {
  return Task.from(
    () =>
      ...
      }).then((res) => res.body.json().then((res) => [res].flat())) as Promise<
        GitHubFile[]
      >,
    (e) =>
      GetDirFailedError({
    ...
    return Task.parallel(
      files.map((file: GitHubFile) => {
        return get<T>(`${path}/${file.name}`).tap(() => {
          console.log(
            `Fetching ${path}/${file.name} (${++count}/${files.length})`,
          );
        });
      }),
      50,
