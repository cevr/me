...
type TaskRecordStatic = {
  ...
  isPending<Result>(record: TaskRecord<Result>): record is PendingTaskRecord<Result>;
  isInProgress<Result>(record: TaskRecord<Result>): record is InProgressTaskRecord<Result>;
  isDone<Result>(record: TaskRecord<Result>): record is DoneTaskRecord<Result>;
  isError<Result>(record: TaskRecord<Result>): record is ErrorTaskRecord<Result>;

  start<Result>(record: TaskRecord<Result>): asserts record is InProgressTaskRecord<Result>;
  finish<Result>(record: TaskRecord<Result>, result: Result): asserts record is DoneTaskRecord<Result>;
  fail<Result>(record: TaskRecord<Result>, error: unknown): asserts record is ErrorTaskRecord<Result>;
};
...
export class TaskQueue<Result> {
  ...
  private subscriberMap: Map<string, Set<(status: TaskStatus) => void>> = new Map();
  ...
  private async processQueue() {
    ...
    while (this.queue.size > 0) {
      const [id, record] = this.queue.entries().next().value as [string, TaskRecord<Result>];
      try {
  ...
  async add<T extends Result>(id: string, task: () => Promise<T>): Promise<T> {
    if (this.queue.has(id)) {
      ...
      return (this.queue.get(id) as PendingTaskRecord<Result> | InProgressTaskRecord<Result>)!.data.deferred
